(() => {
  ...
  function monsterCard(m,opt={}){
    ...
    <div class="card mon monsterCardV78 ${detailMode ? "tapMonsterCardV78" : ""} ${opt.pick ? "pick" : ""} ${m.locked ? "lockedMon" : ""} ${compact ? "monCompact" : ""}" ${detailAttr}>
      ...
      <div>
        <div class="name">${U.esc(m.nickname)} <span class="tag">${d.rank}</span><span class="type">${D.TYPES[d.type]}</span>${sizeBadge(d)}${m.mutation ? `<span class="mutationBadge">${U.esc(S.mutationTitleName(m))}突然変異</span>` : ""}${lockBadge}</div>
