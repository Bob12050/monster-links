  ...
  function homeHtml(){
    ...
    <main class="homeV82 homeV817 homeV821 homeV834 homeV848 homeV851 homeV853">
      <section class="homePhoneV853" style="--home-bg:url('${U.esc(backgroundAssetUrl(baseCampBackground))}')">
        ...
        <div class="homeSpeechV853">おかえりなさい！</div>
        ...
        <section class="homePartyPanelV853" aria-label="パーティ">
          <header><span>パーティ</span><small>所持 ${state.party.length + state.box.length}体</small></header>
          <div class="homePartyRailV853">
            ...
            ${Array.from({length:Math.max(0,5 - partyRail.length)},()=>`
              <button class="empty" onclick="Game.setView('monsters')" aria-label="パーティ編成">
                <span>${ico("monster")}</span><b>+</b><small>空き</small>
              </button>`).join("")}
          </div>
        </section>

        <button class="homeEventBannerV853" onclick="Game.setView('${goal.view}')">
          <span><small>次の任務</small><b>${U.esc(goal.title)}</b><em>${U.esc(goal.text)}</em></span>
          <i>${ico("star")}</i>
        </button>
        ...
        <div class="homeMetaV853 hubWorldV82">
          <button onclick="Game.setView('fusion')"><small>配合所</small><b>${recipeDone}/${recipeTotal}</b></button>
          <button onclick="Game.setView('dex')"><small>図鑑</small><b>${dex.discovered}/${dex.total}</b></button>
          <button onclick="Game.setView('shop')"><small>道具</small><b>${bagCount}</b></button>
          <button onclick="Game.setView('arena')"><small>闘技場</small><b>${arenaClears}</b></button>
          <button onclick="Game.setView('quest')"><small>お守り</small><b>${scoutCharm}</b></button>
