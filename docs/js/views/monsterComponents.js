(() => {
  ...
  function monsterCard(m,opt={}){
    ...
    return `
    <div class="card mon ${opt.pick ? "pick" : ""} ${compact ? "monCompact" : ""}">
      ${V.monsterVisual(m.id,'face')}
      <div>
        <div class="name">${U.esc(m.nickname)} <span class="tag">${d.rank}</span><span class="type">${D.TYPES[d.type]}</span></div>
  ...
  function monsterActions(m,opt){
    ...
    if(opt.mode === "fusion"){
      return `<div class="actions">
        <button class="gold" onclick="Game.pickFusion('${m.uid}')">${opt.pick ? "選択中" : "配合に選ぶ"}</button>
      </div>`;
    }
    return "";
  }
