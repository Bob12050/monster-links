...
- `meadow / cave / brook / volcano` の通常敵Lv上限は `1 / 5 / 7 / 11`。
- 同4地域のボスは `js/core/stages.js` に完全な `boost` オブジェクトを持ち、後半共通既定値へ戻さない。
- 火山の `firstClearExpBonus:2500` は初回撃破時だけ。再戦・ボス再スカウトでは付与しない。
- 火山の `offenseEmergencyHealRate:.30` はボス戦の攻撃優先作戦だけ。他地域・通常戦・他作戦へ広げない。
- 敵ID、ボス表示Lv、必要勝利数、通常報酬、モンスター基礎能力、配合、セーブschemaはこの調整では不変。
- 星晶の塔以降のステージ値を序盤調整と一緒に変更しない。接続は初回撃破EXPで補う。
...
ぷる系固定6体は現在の見た目を維持し、それ以外はA.61以降の成熟した有機造形を基準に、ユーザー承認を経て少数ずつ再設計する方針です。
...
ランクは装飾量と象徴の数、`size`はキャンバス内の物理的な占有率を決めます。AランクがSランク以上に豪華・巨大に見える構図は禁止します。

```text
S: 伝説級。大輪郭に加え、翼・第三眼・特殊角など2〜3個の主要な象徴を許可
A: 強敵級。主要な象徴は1個に絞り、巨大な光輪・多重翼・過剰な宝飾は避ける
B/C: 中堅級。系統を示す角・武器・属性部位のうち1個を中心にする
D以下: 入り口級。輪郭と1色の属性差で識別し、弱々しくはしない
```

同じ正方形画像へ最大化して階級差を消さないこと。v8.6-A.64の基準例は、S・3枠の`chaoswyrm`が前景最大900px、A・2枠の`venomhydra`が820px、A・1枠の`pearlseraph`が760pxです。シルエットに応じて微調整できますが、上位rank/sizeより大きく見せないことを優先します。
...
背景透過
全身中央配置
前景の最大辺900px、各辺およそ6%以上の安全余白
文字、ロゴ、枠、透かしなし
```

### 2026年6月7日時点の維持対象（履歴）

以下は2026年6月7日に確認・実装済みとした当時の一覧です。現在は個別承認を優先し、`venomchimera` と旧 `demonlord` はv8.6-A.61、`celestiseraph` はv8.6-A.62、`omegaframe` はv8.6-A.63、`chaoswyrm` はv8.6-A.64、`phoenixdrake` と `eclipsewolf` はv8.6-A.66、現行の `demonlord` と `doomgazer` はv8.6-A.67で正式刷新済みです。
...
`plim`の名前は「ぷるミン」のまま維持します。ユーザー承認済みのため、`plim`, `kingplim`, `poisonplim`, `cloudplim`, `gearslime`, `titanplim` の6体は全面刷新から除外し、現在の見た目を維持します。

### 全面刷新対象

ぷる系固定6体を除くモンスターは、既存画像が技術上`formal_png`でも、露出と優先度に応じて少数ずつ再デザイン対象にできます。
...
```text
ぷる系固定6体を除くキャラクターアートの優先度別・少数バッチ再設計
序盤8体のデザイン作成
初回プレイの案内とチュートリアル改善
モンスター詳細画面の情報整理
配合画面の親選択フロー短縮
