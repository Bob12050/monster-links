...
- 通常画面のAtlasシェルは約430pxのスマホキャンバスを維持する。外側ブラウザの幅だけを根拠に、内部カードをPC用の複数列へ戻さない。
- `stage / quest / monsters` の主要カードは、外側がPC幅でもスマホキャンバス内で読める列数・寸法を維持する。
- 高さ520px以下の横向きでは、HOMEと戦闘を重ねずスクロール可能にする。戦闘画面は利用可能な横幅まで広げ、コマンドを潰さない。
- 高さ620px以上の縦スマホ戦闘では、主要操作を44px以上のタップ領域として維持する。
- `battle / reward` には通常の下部ナビを表示しない。戦闘結果は詳細を独立スクロール領域、操作を常時到達可能な下段として扱う。
- 任務のopenセクションと受取可能報酬は内容高を保持し、固定高グリッド内で縮めて隠さない。
- 配合の固定実行バーは親2体を選んだ時だけ表示する。0/2・1/2では既存の親選択導線を優先する。
- safe-area insetはAtlasシェル側で一度だけ計上し、下部dockで重複加算しない。
- 表示修正ではview ID、`Game.*` callback、ゲームデータ、戦闘・進行・配合仕様、セーブschema/keyを変更しない。
